...
Feature: Create a New Country
  ...
  Scenario Outline: Valid request to create a new country
    Given path 'country'
  ...
  Scenario: Invalid request due to missing headers
    Given path 'country'
    And request
      """
      {
  ...
  Scenario Outline: Invalid request due to missing required body property
    Given path 'country'
    And header app-identifier = '<app_identifier>'
    And header customer-secret-key = '<customer_secret_key>'
    And request
    ...
    Examples:
      | app_identifier                                                 | customer_secret_key                                            | code  | currencies             | default_currency | is_active |
      | 'd645d2b40811f5672c93794d8622da45588f32cdb2ed85336475bf479e5d' | '12546580645c21b8a3529680f455f715b6b6829da8976bdfd2e9d51181fe' | 'IND' | ['INR', 'USD', 'EURO'] | 'INR'            |         1 |

  Scenario Outline: Valid request with nullable name property
    Given path 'country'
    And header app-identifier = '<app_identifier>'
    And header customer-secret-key = '<customer_secret_key>'
    And request
